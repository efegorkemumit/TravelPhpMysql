...
if($_SERVER["REQUEST_METHOD"]=="POST"){
	$id = $_POST['id'];
	$stmt = $conn->prepare("DELETE FROM bookings_hotels  WHERE `booking_id`=:id");
...
try {
    ...
    FROM     bookings_hotels bt    INNER JOIN    users u ON bt.user_id = u.id    INNER JOIN    hotels t ON bt.hotel_id = t.id     ");
    $stmt ->execute();
