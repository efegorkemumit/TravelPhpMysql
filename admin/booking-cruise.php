...
if($_SERVER["REQUEST_METHOD"]=="POST"){
	$id = $_POST['id'];
	$stmt = $conn->prepare("DELETE FROM bookings_cruise  WHERE `booking_id`=:id");
...
try {
    ...
    FROM     bookings_cruise bt    INNER JOIN    users u ON bt.user_id = u.id    INNER JOIN    cruise t ON bt.cruise_id = t.id     ");
    $stmt ->execute();
