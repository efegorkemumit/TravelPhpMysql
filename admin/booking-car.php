...
if($_SERVER["REQUEST_METHOD"]=="POST"){
	$id = $_POST['id'];
	$stmt = $conn->prepare("DELETE FROM bookings_car  WHERE `booking_id`=:id");
...
try {
    ...
    FROM     bookings_car bt    INNER JOIN    users u ON bt.user_id = u.id    INNER JOIN    car t ON bt.car_id = t.id     ");
    $stmt ->execute();
