...
if($_SERVER["REQUEST_METHOD"]=="POST"){
	$id = $_POST['id'];
	$stmt = $conn->prepare("DELETE FROM bookings_trip  WHERE `booking_id`=:id");
...
try {
    ...
    FROM     bookings_trip bt    INNER JOIN    users u ON bt.user_id = u.id    INNER JOIN    trips t ON bt.trip_id = t.id     ");
    $stmt ->execute();
