...
if($_SERVER["REQUEST_METHOD"]=="POST"){
	$id = $_POST['id'];
	$stmt = $conn->prepare("DELETE FROM bookings_flight  WHERE `booking_id`=:id");
...
try {
    ...
    FROM     bookings_flight bt    INNER JOIN    users u ON bt.user_id = u.id    INNER JOIN    flight t ON bt.flight_id = t.id     ");
    $stmt ->execute();
